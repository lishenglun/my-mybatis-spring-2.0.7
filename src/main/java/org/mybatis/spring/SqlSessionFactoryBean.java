 ...
 * 构建SqlSessionFactory
 *
 * 题外：当前类，有意义的代码是：SqlSessionFactoryBean#afterPropertiesSet()，里面创建了SqlSessionFactory；
 * 像getObject()只是获取SqlSessionFactory；onApplicationEvent()只是解析剩下的之前解析出错的ResultMap、引用缓存、sql标签、sql注解
 *
...
public class SqlSessionFactoryBean
  ...
  protected SqlSessionFactory buildSqlSessionFactory() throws Exception {
    ...
    // 配置了Configuration对象
    // 题外：我们可以直接在sprig配置文件中，配置Configuration bean，然后在配置SqlSessionFactoryBean bean的时候，引入Configuration bean，
    // >>> 这样就有了Configuration对象
    if (this.configuration != null) {
      targetConfiguration = this.configuration;
      // 如果Configuration中没有变量，那么就设置变量
    ...
    // 没配置Configuration对象，但是有mybatis配置文件，则创建XMLConfigBuilder去解析mybatis配置文件
    else if (this.configLocation != null) {
      // ⚠️里面创建了一个Configuration对象
      xmlConfigBuilder = new XMLConfigBuilder(this.configLocation.getInputStream()/* 获取配置文件的输入流 */, null,
        this.configurationProperties);
      targetConfiguration = xmlConfigBuilder.getConfiguration();
    }
    // 没有配置Configuration对象，也没有配置mybatis配置文件，则主动创建一个Configuration对象
    else {
      LOGGER.debug(
          () -> "Property 'configuration' or 'configLocation' not specified, using default MyBatis Configuration");
      // ⚠️主动创建一个Configuration对象
      targetConfiguration = new Configuration();
      Optional.ofNullable(this.configurationProperties).ifPresent(targetConfiguration::setVariables);
    }

    /* 2、往Configuration里面，设置配置的属性（这些属性是通过整合spring，在配置SqlSessionFactoryBean bean时，注入的属性） */
    ...
    if (hasLength(this.typeAliasesPackage)) {
      // 搜索包下this.typeAliasesSuperType类型的类
    ...
    /* 3、解析mybatis配置文件 */
    // 如果XMLConfigBuilder不为null，代表我们配置了mybatis配置文件，所以就解析mybatis配置文件
    ...
    targetConfiguration.setEnvironment(new Environment(this.environment,
        // ⚠️事务工厂 = SpringManagedTransactionFactory
        this.transactionFactory == null ? new SpringManagedTransactionFactory() : this.transactionFactory,
        this.dataSource));

    /* 4、解析配置的所有的dao.xml文件 */

    // 如果mapperLocations不为null，代表我们配置了dao.xml文件，那么就解析所有的dao.xml文件
    ...
    /* 5、创建SqlSessionFactory（DefaultSqlSessionFactory） */
  ...
  public SqlSessionFactory getObject() throws Exception {
    // 一般，sqlSessionFactory是存在，因为在实例化SqlSessionFactoryBean的时候，afterPropertiesSet()已经执行了，所以就有了sqlSessionFactory
  ...
  public void onApplicationEvent(ApplicationEvent event) {
    if (failFast && event instanceof ContextRefreshedEvent) {
      ...
      调用该方法只为了一个目的：
      解析剩下的之前解析出错的ResultMap、引用缓存、sql标签、sql注解（sql标签和sql注解是构建MappedStatement，放入Configuration.mappedStatements）
  ...
  /**
   * 搜索包下所有对应类型的类。如果搜索的类型为null，则代表获取包下的所有类。
   *
   * @param packagePatterns     包
   * @param assignableType      搜索的归属类型
   */
  private Set<Class<?>> scanClasses(String packagePatterns, Class<?> assignableType) throws IOException {
    // 存储包下，搜索到的合适的类
    Set<Class<?>> classes = new HashSet<>();

    String[] packagePatternArray = tokenizeToStringArray/* 标记为字符串数组 */(packagePatterns,
        ConfigurableApplicationContext.CONFIG_LOCATION_DELIMITERS/* ,; \t\n */);

    for (String packagePattern : packagePatternArray) {

      // 获取包下的所有类文件，一个类文件对应了Resource对象
      Resource[] resources = RESOURCE_PATTERN_RESOLVER.getResources(ResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX/* classpath*: */
          + ClassUtils.convertClassNameToResourcePath(packagePattern) + "/**/*.class");

      // 遍历类文件
      for (Resource resource : resources) {
        try {
          // 通过类文件，获取类对应的ClassMetadata
          ClassMetadata classMetadata = METADATA_READER_FACTORY.getMetadataReader(resource).getClassMetadata();
          // 通过ClassMetadata获取类的全限定名，然后获取类Class
          Class<?> clazz = Resources.classForName(classMetadata.getClassName());

          // 搜索的归属类型为null || 存在搜索的归属类型，但是当前类归属于搜索的归属类型，也就是说，当前类是搜索的归属类型，或者是它的子类，或者是它的实现类，就都成立
          // 题外：搜索的归属类型为null，代表获取包下的所有类
          // 题外：搜索的归属类型不为null，代表获取包下对应类型的类，要么是这个类，要么是这个的子类，要么是这个的实现类
